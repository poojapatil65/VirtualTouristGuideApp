...
public class Church extends AppCompatActivity {
    ...
    private Context mContext2=Church.this;
    private RecyclerView recyclerView;

    private ArrayList<Member3> imageslist2;
    private MyAdapter4 recyclerAdapter;
    @Override
    ...
    private void  init()
    {
        ...
        Query query=reference.child("Member3");
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot:dataSnapshot.getChildren())
                {
                    ...
                    member2.setChurchName(snapshot.child("churchName").getValue().toString());
                    member2.setCountry(snapshot.child("country").getValue().toString());
                    member2.setLocation(snapshot.child("location").getValue().toString());
                    member2.setYear_establish(snapshot.child("year_establish").getValue().toString());
                    member2.setChurch_info(snapshot.child("church_info").getValue().toString());
                    member2.setImageAddress(snapshot.child("imageAddress").getValue().toString());
                    imageslist2.add(member2);
                }
                recyclerAdapter=new MyAdapter4(mContext2,imageslist2);
