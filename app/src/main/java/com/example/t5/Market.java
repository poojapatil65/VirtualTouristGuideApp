...
public class Market extends AppCompatActivity {
    ...
    private Context mContext2=Market.this;
    private RecyclerView recyclerView;

    private ArrayList<Member6> imageslist2;
    private MyAdapter3 recyclerAdapter;
    @Override
    ...
    private void  init()
    {
        ...
        Query query=reference.child("Member6");
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot:dataSnapshot.getChildren())
                {
                    ...
                    member2.setMarketName(snapshot.child("marketName").getValue().toString());
                    member2.setCountry(snapshot.child("country").getValue().toString());
                    member2.setLocation(snapshot.child("location").getValue().toString());
                    member2.setBuy(snapshot.child("buy").getValue().toString());
                    member2.setMarket_info(snapshot.child("market_info").getValue().toString());
                    member2.setImageAddress(snapshot.child("imageAddress").getValue().toString());
                    imageslist2.add(member2);
                }
                recyclerAdapter=new MyAdapter3(mContext2,imageslist2);
